  ...
  return (
    <>
      <Box  style={{display:"flex",justifyContent:"space-around"}}>
        <Box>
        <h1 style={{display:"flex"}}>Sort By Year/Rating: <SortByYear  sort={sort} setSort={setSort} /></h1>
        </Box>
        <Box>
        <h1 style={{display:"flex"}}>Filter By Genres : <FitlerByGenres genres={genres} setGenres={setGenres} /></h1>
        </Box>
      </Box>
      <Container>
        {sort.length===0 && genres.length===0 ? < >{movies.map((item) => (
